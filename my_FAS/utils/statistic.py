...
def calculate(probs, labels):
    ...
    APCER = 1.0 if (FP + TN == 0) else FP / float(FP + TN)   # false negative rate    (attack presentation classification error rate)
    NPCER = 1.0 if (FN + TP == 0) else FN / float(FN + TP)   # false positive rate     (normal presentation classification error rate)
    ACER = (APCER + NPCER) / 2.0    # average classification error rate  (ACER)
...
def get_threshold(probs, grid_density):
    ...
    thresholds = []
    step = float((Max - Min)/grid_density)
    for i in range(grid_density+1):
        thresholds.append(Min+step*i)
    # for i in range(grid_density + 1):
    #     thresholds.append(0.0 + i * 1.0 / float(grid_density))
    # thresholds.append(1.1)
    return thresholds

def get_EER_states(probs, labels, grid_density = 5000):
    ...
    return EER, thr, FRR_list, FAR_list, TPR_list
