...
if __name__ == '__main__':
    embeddings = torch.rand((4, 8))
    labels = torch.randint(0, 3, (4,))
    pairwise_dist = _pairwise_distance(embeddings, squared=False)
    anc_pos_dist = pairwise_dist.unsqueeze(dim=2)
    anc_neg_dist = pairwise_dist.unsqueeze(dim=1)
    loss = anc_pos_dist - anc_neg_dist + 0.1
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    # Check that i, j and k are distinct
    indices_not_same = torch.eye(labels.shape[0]).to(device).byte() ^ 1  # equal 1 is 0, unequal 1 is 1
    ...
    i_equal_j = torch.unsqueeze(label_equal, 2)
    i_equal_k = torch.unsqueeze(label_equal, 1)
    # i_equal_k_long = torch.LongTensor(i_equal_k)

    print(i_equal_k, i_equal_k ^ True)
